...
use DevinGray\LaraTeams\Events\UserJoinedTeam;
use DevinGray\LaraTeams\Events\UserLeftTeam;
...
trait HasTeams
{
    ...
    {
        return $this->belongsToMany( Config::get( 'larateams.team_model' ),Config::get( 'larateams.team_user_table' ), 'user_id', 'team_id' )->withTimestamps();
    }
    ...
    {
        return $this->hasOne( Config::get( 'larateams.team_model' ), 'id', 'current_team_id' );
    }
    ...
    {
        return $this->hasMany( Config::get('larateams.invite_model'), 'email', 'email' );
    }
    ...
    public static function bootUserHasTeams()
    {
        ...
        {
            if (!method_exists( Config::get( 'larateams.user_model' ), 'bootSoftDeletes' ) )
            {
                $user->teams()->sync( [ ] );
            }
            return true;
